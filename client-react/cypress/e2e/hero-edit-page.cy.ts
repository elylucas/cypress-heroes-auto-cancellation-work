  ...
  describe('when admin user is logged in', () => {
    ...
    it('editing hero should save and display on home page', () => {
      cy.get<Prisma.Hero>('@newHero').then((newHero) => {
        ...
        cy.get('@heroCard').find('[data-cy=price]').should('contain', 'a12');
        cy.get('@heroCard').find('[data-cy=fans]').should('contain', 'a34');
        cy.get('@heroCard').find('[data-cy=saves]').should('contain', 'a56');
        cy.get('@heroCard')
          .find('[data-cy=powers]')
